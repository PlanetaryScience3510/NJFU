 ...
 */
template <int Degree>
class TwoClique : public Clique {
public:
    /** Construct a TwoClique on a set of variables.
     */
    TwoClique(const std::vector<VarId>& nodes, REAL same_cost,
                REAL diff_cost)
            : m_sameCost(same_cost),
              m_diffCost(diff_cost)
    {
    ...
    /** Return the two order energy for a given labeling
     */
    virtual REAL energy(const Label* labels) const override {
        const Label l = labels[0];
        for (int i = 1; i < Degree; ++i) {
 ...
 * 定义鲁棒的Potts模型
 */
class RubustPottsClique : public Clique {
public:
    /** Construct a RubustPottsClique on a set of variables.
     */
    RubustPottsClique(const std::vector<VarId>& nodes, int q,
                REAL r_max)
            : Q(q),
              R_max(r_max)
    {
    ...
    /** Return the rubust potts energy for a given labeling
     */
    virtual REAL energy(const Label* labels) const override {
        int num0=0;
        int num1=0;
        ...
        if((num0+num1+num2-num_max)<=Q)
            return R_max*((float)(num0+num1+num2-num_max))/Q;

        //std::cout<<"返回能量函数"<<std::endl;
        return R_max;
...
inline void MultilabelEnergy::addUnaryTerm(VarId i,
    ...
    for (Label l = 0; l < coeffs.size(); ++l) {
        ...
        //std::cout << i << "  " << l << "   "<<coeffs[l] << std::endl;
    }
}
...
//        std::cout<<"m_cliques.size(): "<<m_cliques.size()<<std::endl;
//    }
    m_cliques.push_back(std::move(c));
    //std::cout<<"m_cliques.size(): "<<m_cliques.size()<<std::endl;
}
