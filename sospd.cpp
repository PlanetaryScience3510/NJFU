...
void AlphaProposal(int niter, const std::vector<size_t>& current,
                   std::vector<size_t>& proposed) {
    for (size_t &l : proposed) {
...
void SoSPD<Flow>::SetupGraph(Flow& crf) {
    ...
    std::cout<<"高阶团数量: "<<m_energy->cliques().size()<<std::endl;
...
bool SoSPD<Flow>::InitialFusionLabeling() {
    ...
    for (size_t i = 0; i < m_labels.size(); ++i) {
        ...
        if (m_labels[i] != m_fusion_labels[i]){
            allDiff = true;
            //std::cout<<m_labels[i]<<"   "<<m_fusion_labels[i]<<std::endl;
        }
